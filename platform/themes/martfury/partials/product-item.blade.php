...
<div class="ps-product__thumbnail">
  ...
  <div class="ps-product__badges">
    ...
    <span class="ps-product__badge" @if ($label->color) style="background-color: {{ $label->color }} !important;"
      @endif>{{ $label->name }}</span>
    @endforeach
  </div>
  @else
  ...
  <ul class="ps-product__actions">
    ...
    <li><a class="add-to-cart-button" data-id="{{ $product->id }}" href="#"
        data-url="{{ route('public.cart.add-to-cart') }}" title="{{ __('Add To Cart') }}"><i class="icon-bag2"></i></a>
    </li>
    @endif
    <li><a class="js-quick-view-button" href="#" data-url="{{ route('public.ajax.quick-view', $product->id) }}"
        title="{{ __('Quick View') }}"><i class="icon-eye"></i></a></li>
